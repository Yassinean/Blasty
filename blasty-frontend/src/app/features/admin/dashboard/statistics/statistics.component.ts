...
export class StatisticsComponent implements OnInit, AfterViewInit {
  ...
  loadData(): void {
    ...
    this.parkingService.getAllParkings().subscribe({
      ...
      error: (error) => {
        this.toastService.showError('',"Erreur lors du chargement des données des parkings");
  ...
  loadOccupancyData(parkingIds: number[]): void {
    const occupancyRequests = parkingIds.map(id => this.parkingService.getParkingOccupancy());
    
    forkJoin(occupancyRequests).pipe(
      catchError(error => {
  ...
  loadRevenueData(parkingIds: number[]): void {
    const revenueRequests = parkingIds.map(id => this.parkingService.getParkingRevenue(id, this.selectedPeriod));
    
    forkJoin(revenueRequests).pipe(
      catchError(error => {
